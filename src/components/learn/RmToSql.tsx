  ...
  const validateSql = useCallback(async () => {
    ...
    try {
      ...
      if (!execResult.success) {
        setResult({
          success: false,
          error: execResult.error || t("unknownError"),
          rowsModified: 0,
        });
        setIsCorrect(false);
        db.close();
      ...
      for (const expectedTable of data.tables) {
        ...
        for (const expectedCol of expectedTable.columns) {
          if (!existingCols.has(expectedCol.name)) {
            errors.push(t("columnMissing", { col: expectedCol.name, table: expectedTable.name }));
            allCorrect = false;
          }
        }
      }
      ...
      if (allCorrect) {
        ...
        setResult({
          success: true,
          resultset: { columns: [{ name: "Result", type: "text" }], rows: [{ Result: t("allTablesCorrect") }] },
          rowsModified: 0,
          statementType: "DDL",
        });
      } else {
    ...
    <div className={cn("space-y-4", className)}>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <label className="text-sm font-medium text-ink">{t("yourSqlSolution")}</label>
          <button
      ...
      {result && (
        <div className="space-y-2">
          {isCorrect && (
            <SuccessCelebration
              message={t("correctExclamation")}
              submessage={t("rmCorrectSubmessage")}
            />
          )}
          ...
          {!result.success && (
            <div className="p-3 rounded-lg bg-error/10 border border-error/30">
              <p className="text-xs font-semibold text-error">{t("error")}</p>
