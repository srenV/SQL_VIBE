...
const DATASET_IDS = ["shop", "fitness", "hr", "tickets", "banking", "streaming", "logs"];
...
function buildPredict(prefix, opts) {
  const options = opts.options
    .map((o, i) => `      { text: ${JSON.stringify(o.text)}, isCorrect: ${o.isCorrect} }`)
    .join(",\n");
  ...
  const whereTemplates = [
    {
      ...
      desc: (table, col, val) =>
        `Zeige alle Zeilen aus \`${table}\`, bei denen \`${col}\` den Wert '${val}' hat.`,
    ...
    },
    {
      ...
      desc: (table, col, val) =>
        `Zeige alle Zeilen aus \`${table}\`, bei denen \`${col}\` groesser als ${val} ist.`,
      query: (table, col, val) => `SELECT * FROM ${table} WHERE ${col} > ${val};`,
      hint1: (table, col, val) => `Verwende \`WHERE ${col} > ${val}\`.`,
    ...
    },
    {
      ...
      desc: (table, col, val) =>
        `Zeige alle Zeilen aus \`${table}\`, bei denen \`${col}\` ungleich '${val}' ist.`,
      query: (table, col, val) => `SELECT * FROM ${table} WHERE ${col} <> '${val}';`,
    ...
    },
    {
      ...
      desc: (table, col, val) =>
        `Zeige alle Zeilen aus \`${table}\`, bei denen \`${col}\` zwischen ${val} und einem groesseren Wert liegt.`,
      query: (table, col, val) =>
        `SELECT * FROM ${table} WHERE ${col} BETWEEN ${val} AND ${parseInt(val) + 1000};`,
      hint1: (table, col, val) =>
        `Verwende \`BETWEEN ${val} AND ${parseInt(val) + 1000}\`.`,
    ...
    },
    {
      ...
      desc: (table, col, val) =>
        `Zeige alle Zeilen aus \`${table}\`, bei denen \`${col}\` in einer Liste ist.`,
      query: (table, col, val) => {
        const vals = val
          .split(",")
          .map((v) => `'${v.trim()}'`)
    ...
    },
    {
      ...
      desc: (table, col, val) =>
        `Zeige alle Zeilen aus \`${table}\`, bei denen \`${col}\` mit '${val}' beginnt.`,
    ...
    },
    {
      ...
      desc: (table, col, val) =>
        `Zeige alle Zeilen aus \`${table}\`, bei denen \`${col}\` NULL ist.`,
  ...
  const templates = [
    {
      ...
      desc: (table, col) =>
        `Gib alle Zeilen aus \`${table}\` aus, sortiert nach \`${col}\` aufsteigend.`,
    ...
    },
    {
      ...
      desc: (table, col) =>
        `Gib alle Zeilen aus \`${table}\` aus, sortiert nach \`${col}\` absteigend.`,
    ...
    },
    {
      ...
      desc: (table, col) =>
        `Gib die ersten 5 Zeilen aus \`${table}\` aus, sortiert nach \`${col}\` absteigend.`,
    ...
    },
    {
      ...
      desc: (table, col) =>
        `Gib die ersten 3 Zeilen aus \`${table}\` aus, sortiert nach \`${col}\` aufsteigend.`,
...
function generateAggregationExercises() {
  ...
  const templates = [
    ...
    },
    {
      ...
      desc: (table, col) =>
        `Berechne den Durchschnitt der Spalte \`${col}\` in \`${table}\`.`,
    ...
    },
    {
      ...
      desc: (table, col) =>
        `Finde den hoechsten Wert der Spalte \`${col}\` in \`${table}\`.`,
    ...
    },
    {
      ...
      desc: (table, col) =>
        `Finde den niedrigsten Wert der Spalte \`${col}\` in \`${table}\`.`,
    ...
    },
    {
      ...
      desc: (table, col) =>
        `Zaehle die Zeilen in \`${table}\` gruppiert nach \`${col}\`.`,
      query: (table, col) =>
        `SELECT ${col}, COUNT(*) AS anzahl FROM ${table} GROUP BY ${col};`,
    ...
    },
    {
      ...
      desc: (table, col) =>
        `Gruppiere \`${table}\` nach \`${col}\` und zeige nur Gruppen mit mehr als einem Eintrag.`,
      query: (table, col) =>
        `SELECT ${col}, COUNT(*) AS anzahl FROM ${table} GROUP BY ${col} HAVING COUNT(*) > 1;`,
  ...
  for (const dsId of datasets) {
    ...
    for (const table of ds.tables.slice(0, 2)) {
      ...
      const useCol =
        testTitle.includes("Gruppieren") || testTitle.includes("HAVING") ? groupCol : col;
      exercises.push(
        buildWrite("agg", {
...
function generateJoinExercises() {
  ...
  const joinPairs = [
    ...
    },
    {
      ...
      t1: "produkte",
      t2: "kategorien",
      on: "produkte.kategorie_id = kategorien.id",
      col1: "produkte.name",
      col2: "kategorien.name",
    },
    {
      ds: "shop",
      t1: "bestellungen",
      t2: "zahlungen",
      on: "bestellungen.id = zahlungen.bestellung_id",
      col1: "status",
      col2: "betrag",
    },
    {
      ...
      t1: "nutzer",
      t2: "workouts",
      on: "nutzer.id = workouts.nutzer_id",
      col1: "nutzer.name",
      col2: "dauer_min",
    },
    {
      ...
      t1: "workouts",
      t2: "saetze",
      on: "workouts.id = saetze.workout_id",
      col1: "datum",
      col2: "wiederholungen",
    },
    {
      ...
      t1: "mitarbeiter",
      t2: "abteilungen",
      on: "mitarbeiter.abteilung_id = abteilungen.id",
      col1: "mitarbeiter.name",
      col2: "abteilungen.name",
    },
    {
      ds: "hr",
      t1: "mitarbeiter",
      t2: "urlaub",
      on: "mitarbeiter.id = urlaub.mitarbeiter_id",
      col1: "name",
      col2: "tage",
    },
    {
      ...
      t1: "tickets",
      t2: "agenten",
      on: "tickets.agent_id = agenten.id",
      col1: "tickets.titel",
      col2: "agenten.name",
    },
    {
      ...
      t1: "tickets",
      t2: "kategorien",
      on: "tickets.kategorie_id = kategorien.id",
      col1: "tickets.titel",
      col2: "kategorien.name",
    },
    {
    ...
    },
    {
      ...
      t1: "konten",
      t2: "transaktionen",
      on: "konten.id = transaktionen.konto_id",
      col1: "kontonummer",
      col2: "betrag",
    },
    {
      ...
      t1: "nutzer",
      t2: "watch_history",
      on: "nutzer.id = watch_history.nutzer_id",
      col1: "nutzer.name",
      col2: "fortschritt_prozent",
    },
    {
      ds: "streaming",
      t1: "filme",
      t2: "bewertungen",
      on: "filme.id = bewertungen.film_id",
      col1: "filme.titel",
      col2: "sterne",
    },
    {
  ...
  const templates = [
    {
      ...
      desc: (pair) =>
        `Zeige ${pair.col1} und ${pair.col2} durch einen INNER JOIN zwischen \`${pair.t1}\` und \`${pair.t2}\`.`,
      query: (pair) =>
        `SELECT ${pair.col1}, ${pair.col2} FROM ${pair.t1} INNER JOIN ${pair.t2} ON ${pair.on};`,
    ...
    },
    {
      ...
      desc: (pair) =>
        `Zeige alle Zeilen aus \`${pair.t1}\` und passende aus \`${pair.t2}\` (auch wenn keine Uebereinstimmung).`,
      query: (pair) =>
        `SELECT ${pair.col1}, ${pair.col2} FROM ${pair.t1} LEFT JOIN ${pair.t2} ON ${pair.on};`,
    ...
    },
    {
      ...
      desc: (pair) =>
        `Zeige alle Zeilen aus \`${pair.t2}\` und passende aus \`${pair.t1}\`.`,
      query: (pair) =>
        `SELECT ${pair.col1}, ${pair.col2} FROM ${pair.t1} RIGHT JOIN ${pair.t2} ON ${pair.on};`,
    ...
    },
    {
      ...
      desc: (pair) =>
        `Zaehle, wie viele Zeilen aus \`${pair.t2}\` zu jeder Zeile aus \`${pair.t1}\` passen.`,
      query: (pair) => {
        const pk = pair.t1 + ".id";
    ...
    },
    {
      ...
      desc: (pair) =>
        `Verbinde \`${pair.t1}\` mit sich selbst, um hierarchische Beziehungen zu zeigen.`,
      query: (pair) => {
  ...
  const templates = [
    {
      ...
      desc: (item) =>
        `Zeige alle Zeilen aus \`${item.outer}\`, fuer die es mindestens eine passende Zeile in \`${item.inner}\` gibt.`,
      query: (item) =>
        `SELECT ${item.select} FROM ${item.outer} WHERE EXISTS (SELECT 1 FROM ${item.inner} WHERE ${item.inner}.${item.innerCol} = ${item.outer}.${item.outerCol});`,
    ...
    },
    {
      ...
      desc: (item) =>
        `Zeige Zeilen aus \`${item.outer}\`, deren \`${item.outerCol}\` in einer Liste aus \`${item.inner}\` vorkommt.`,
      query: (item) =>
        `SELECT ${item.select} FROM ${item.outer} WHERE ${item.outerCol} IN (SELECT ${item.innerCol} FROM ${item.inner});`,
    ...
    },
    {
      ...
      desc: (item) =>
        `Zaehle fuer jede Zeile in \`${item.outer}\` die zugehoerigen Zeilen in \`${item.inner}\`.`,
      query: (item) =>
        `SELECT ${item.select}, (SELECT COUNT(*) FROM ${item.inner} WHERE ${item.inner}.${item.innerCol} = ${item.outer}.${item.outerCol}) AS anzahl FROM ${item.outer};`,
...
function generateDebugExercises() {
  ...
  const debugItems = [
    {
      ...
      table: "kunden",
      broken:
        "SELECT name, stadt FROM kunden WHERE stadt = 'Berlin' AND stadt = 'Muenchen';",
      fixed:
        "SELECT name, stadt FROM kunden WHERE stadt = 'Berlin' OR stadt = 'Muenchen';",
    },
    {
      ds: "shop",
      table: "produkte",
    ...
    },
    {
      ...
      table: "tickets",
      broken:
        "SELECT titel, status FROM tickets WHERE status = 'offen' OR status = 'bearbeitung' AND prioritaet = 'hoch';",
      fixed:
        "SELECT titel, status FROM tickets WHERE (status = 'offen' OR status = 'bearbeitung') AND prioritaet = 'hoch';",
    },
    {
      ...
      fixed:
        "SELECT konto_id, SUM(betrag) FROM transaktionen WHERE typ = 'ausgang' GROUP BY konto_id;",
    },
    {
      ds: "streaming",
      table: "filme",
      broken:
        "SELECT titel FROM filme WHERE jahr > 2000 AND genre = 'Drama' OR genre = 'Action';",
      fixed:
        "SELECT titel FROM filme WHERE jahr > 2000 AND (genre = 'Drama' OR genre = 'Action');",
    },
    {
      ...
      table: "events",
      broken:
        "SELECT session_id, COUNT(*) FROM events GROUP BY session_id HAVING COUNT > 5;",
      fixed:
        "SELECT session_id, COUNT(*) AS anzahl FROM events GROUP BY session_id HAVING COUNT(*) > 5;",
    },
  ];
  ...
  const moreDebug = [
    {
      ds: "shop",
      broken: "SELECT * FROM kunden ORDER BY name;",
      fixed: "SELECT * FROM kunden ORDER BY name ASC;",
      note: "ASC ist optional, aber korrekt.",
    },
    {
      ds: "hr",
      broken: "SELECT abteilung_id, AVG(gehalt) FROM mitarbeiter;",
      fixed: "SELECT abteilung_id, AVG(gehalt) FROM mitarbeiter GROUP BY abteilung_id;",
      note: "GROUP BY fehlt.",
    },
    {
      ds: "fitness",
      broken: "SELECT name FROM nutzer WHERE gewicht_kg = NULL;",
      fixed: "SELECT name FROM nutzer WHERE gewicht_kg IS NULL;",
      note: "IS NULL statt = NULL.",
    },
    {
      ds: "tickets",
      broken: "SELECT titel FROM tickets WHERE prioritaet IN ('hoch', 'kritisch';",
      fixed: "SELECT titel FROM tickets WHERE prioritaet IN ('hoch', 'kritisch');",
      note: "Klammer nicht geschlossen.",
    },
    {
      ds: "banking",
      broken: "SELECT k.name, k.saldo FROM kunden k JOIN konten k ON k.id = k.kunde_id;",
      fixed:
        "SELECT k.name, ko.saldo FROM kunden k JOIN konten ko ON k.id = ko.kunde_id;",
      note: "Alias doppelt verwendet.",
    },
    {
      ds: "streaming",
      broken: "SELECT * FROM filme WHERE bewertung > 4.5;",
      fixed: "SELECT * FROM filme WHERE bewertung > 4.5;",
      note: "Nur als Kontrollaufgabe.",
    },
    {
      ds: "logs",
      broken: "SELECT event_typ FROM events WHERE dauer_ms > 200 AND < 400;",
      fixed: "SELECT event_typ FROM events WHERE dauer_ms > 200 AND dauer_ms < 400;",
      note: "Spaltenname fehlt im zweiten Teil.",
    },
  ];
  ...
  const schemaItems = [
    {
      ds: "shop",
      question:
        "Welche Tabellen muessen verbunden werden, um den Namen eines Kunden und den Namen des gekauften Produkts anzuzeigen?",
      options: [
    ...
    },
    {
      ds: "streaming",
      question:
        "Welche Tabellen braucht man, um zu sehen, welcher Nutzer welchen Film geschaut hat?",
      options: [
